...
export function generateWhopProductData(
  beatTitle: string,
...
function generateDescription(term: LicenseTerms['basic' | 'premium' | 'unlimited'], fileType: string): string {
  const sections = [
    // What you get - start with this as it's most important
    `📁 **What You Get:**\n• High quality ${fileType} file${fileType.includes('+') ? '' : ' of the beat'}`,
    
    // Usage limits - comprehensive license data
    ...
    // What you CAN do - all permissions
    `\n✅ **What You CAN Do:**\n${term.canDo.map((item: string) => `• ${item}`).join('\n')}`,
    
    // What you CANNOT do - all restrictions
    `\n🚫 **What You CANNOT Do:**\n${term.cannotDo.map((item: string) => `• ${item}`).join('\n')}`,
...
export function testWhopProductGeneration() {
  const mockTerms: LicenseTerms = {
    basic: {
